...
<div class="container">
    ...
    <!-- بەشی بەخێرهاتن - WELCOME to 7AJI-IOS -->
    ...
    <div class="admin-content new-style">
        ...
        <div id="adminDashboardView" style="display:none;">
            ...
            <div class="form-group">
                ...
                <input type="text" id="appName" placeholder="بۆ نموونە: Gbox, Scarlet...">
            </div>
            ...
            <div class="form-group">
                <label>🖼️ وێنەی ئەپ (ئایکۆن)</label>
                <div class="image-picker">
                    ...
                    <div class="image-options">
                        ...
                        <input type="url" id="imageUrl" placeholder="یان لینکی وێنە..." style="margin-top:8px; width:100%;">
                    </div>
                </div>
            </div>
...
<script>
    let apps = [
        { name: "Gbox", status: "Working", rating: 4.0, link: "#", description: "", icon: "📦", type: "app" },
        { name: "Scarlet", status: "Working", rating: 5.0, link: "#", description: "", icon: "🔴", type: "app" },
        { name: "Esign", status: "Working", rating: 4.0, link: "#", description: "", icon: "✍️", type: "app" },
        { name: "VideoStar", status: "Unlock all Features", rating: 5.0, link: "#", description: "", icon: "🎬", type: "app" }
    ];
    ...
    function renderApps() {
        ...
        apps.forEach(app => {
            ...
            const icon = app.icon || (app.type === "game" ? "🎮" : "📱");
            card.innerHTML = `
                <div class="app-icon" style="font-size:28px;">${icon}</div>
                <div class="app-info">
    ...
    document.getElementById("selectEmojiBtn").onclick = () => {
        ...
        if(emoji && emoji.length <= 2) {
            document.getElementById("imagePreview").innerHTML = `<span>${emoji}</span>`;
            document.getElementById("imagePreview").style.background = "linear-gradient(135deg, #667eea, #764ba2)";
        }
    };
    ...
    document.getElementById("selectColorBtn").onclick = () => {
        ...
        if(color) {
            document.getElementById("imagePreview").style.background = color;
        }
    };
    ...
    document.getElementById("addAppBtn").onclick = () => {
        ...
        let icon = document.getElementById("imagePreview").innerText.trim();
        if(!icon || icon === "📱") icon = type === "game" ? "🎮" : "📱";
        
        if(!name || !link || isNaN(rating) || rating < 1 || rating > 5) {
            alert("تکایە ناو، لینک و ڕیتینگێکی دروست پڕبکەوە");
            return;
        }
        apps.push({ name, status, rating, link, description, icon, type });
        saveData();
        renderApps();
        renderAdminList();
